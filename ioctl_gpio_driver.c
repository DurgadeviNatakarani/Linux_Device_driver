...
#define GPIO_PIN    529 // GPIO pin number to control
#define GPIO_LED    530
...
//	if(value==1)
//	{
		gpio_direction_output(GPIO_LED,value);
...
{
	static int major;
	major=register_chrdev(0,"ioctl_gpio",&fops);
...
/*    if (alloc_chrdev_region(&dev, 0, 1, DEVICE_NAME) < 0) {
 ...
 //       goto r_cdev_del;
    ...
    if (gpio_request(GPIO_LED, "gpio_ioctl") < 0) {
        pr_err("Failed to request GPIO pin\n");
//        goto r_cdev_del;
    }
